---
layout: 'PostLayout'
title: 'Whats the difference between var, const, and let? And when to use each'
...
# Whats the difference between var, const, and let? And when to use each
...
`var` is scoped to the variable's enclosing function. This means that even if a var is declared within a for loop, it is still accessible throughout the function. Take a look at this example. What do you think will happen?
...
You should use `const` over `let` and `let` over `var`. Using this hierarchy when you declare variables will reduce the number of bugs in your code as well as save you plenty of headaches.

The reason `const` takes preference is because its reference cannot be changed. In the cases that you need to change a variables reference, use let. 

You should pretty much avoid using var as much as you can.